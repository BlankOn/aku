    ...
    def clean_url(self):
        ...
        if not mimetypes.guess_all_extensions(headers.get('Content-Type')):
            raise forms.ValidationError(
                _('The file type is invalid: %s' % type))
...
class AvatarCropForm(forms.Form):
    ...
    def clean(self):
        if int(self.cleaned_data.get('right')) - int(
                self.cleaned_data.get('left')) < 96:
            raise forms.ValidationError(
                _("You must select a portion of the image with a minimum of 96x96 pixels."
                  ))
        else:
            return self.cleaned_data
...
class RegistrationForm(forms.Form):
    username = forms.CharField(
        max_length=255, min_length=3, label=_("Username"))
    ...
    def clean_username(self):
        ...
        if not set(username).issubset(
            ...
            raise forms.ValidationError(
                _("That username has invalid characters. The valid values are letters, numbers and underscore."
                  ))
        if User.objects.filter(username__iexact=username).count() == 0:
            return username
        else:
            raise forms.ValidationError(
                _("The username is already registered."))
    ...
    def clean(self):
        ...
        if self.cleaned_data.get("password1") == self.cleaned_data.get(
            ...
            raise forms.ValidationError(
                _("The passwords inserted are different."))
    ...
    def clean_email(self):
        ...
        try:
            ...
            try:
                ...
                raise forms.ValidationError(
                    _("That e-mail is already being confirmed."))
