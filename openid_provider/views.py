...
def openid_server(req):
    """
    This view is the actual OpenID server - running at the URL pointed to
    by the <link rel="openid.server"> tag.
    """
    ...
    server = Server(
        store, op_endpoint="%s%s" % (host, reverse('openid-provider-root')))
    ...
    if not orequest:
        ...
        if not orequest:
            ...
            return render_to_response(
                'openid_provider/server.html', {'host': host},
    ...
    if orequest.mode in ("checkid_immediate", "checkid_setup"):
        ...
        oresponse = orequest.answer(
            True,
            identity="%s%s" % (host, reverse(
                'openid-provider-identity', args=[req.user.username])))
...
def openid_identity(req, *args):
    return render_to_response(
        'openid_provider/identity.html', {'host': get_base_uri(req)},
        context_instance=RequestContext(req))
