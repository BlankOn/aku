...
class SSHKeysForm(forms.Form):
    ...
    key = forms.CharField(widget=forms.Textarea(attrs=dict(
        attrs_dict, cols=80, rows=5)))
    ...
    def delete(self, user):
        self.deleting = True
        try:
            keyring = SSHKeys.objects.get(user=user,
...
class SSHKeyUploadForm(forms.Form):
    ...
    def clean_file(self):
        if self.files['file'].size > 1024:
            raise forms.ValidationError(
                _("The file you uploaded ('%s') is too big" % self.files[
                    'file']))
        ...
        f.close()
        output = Popen(
            ["ssh-vulnkey", "-v", file], stdout=PIPE).communicate()[0]
        state = 0
        ...
        if state == 0 or self.fingerprint == "":
            message = "The file you uploaded ('%s') is not a valid SSH key file" % self.files[
                'file']
            raise forms.ValidationError(_(message))
        if state == 2:
            message = "The file you uploaded ('%s') is a compromised SSH key file. Replace it immediately" % self.files[
                'file']
            raise forms.ValidationError(_(message))
    ...
    def save(self, user):
        keyring = SSHKeys(
            user=user, fingerprint=self.fingerprint, key=self.text)
        try:
            self.saving = True
            keyring.save()
            self.success = True
